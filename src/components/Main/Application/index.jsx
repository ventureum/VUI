...
class Application extends Component {
  ...
  addItem (src, dist) {
    ...
    for (var i = 0; i < src.length; i++) {
      if (this.state[src[i]] === '' || this.state[src[i]] === undefined) {
        toastr.success('You have missed information, please fill all blanks!')
        return
      }
      newItem[src[i]] = this.state[src[i]]
    }
    if (this.isDup(newItem, this.state[dist])) {
      toastr.success('You have added this item, please don\'t add again!')
      return false
    }
    var newDist = {}
  ...
  render () {
    ...
    for (var i = 0; i < this.state.legalList.length; i++) {
      let item = this.state.legalList[i]
      legalElems.push(
    ...
    for (let i = 0; i < this.state.boardMemberList.length; i++) {
      let item = this.state.boardMemberList[i]
    ...
    var teamMemberElems = []
    for (let i = 0; i < this.state.teamMemberList.length; i++) {
      let item = this.state.teamMemberList[i]
    ...
    var msElems = []
    for (let i = 0; i < this.state.msList.length; i++) {
      let item = this.state.msList[i]
      msElems.push(
    ...
    var objectiveElems = []
    for (let i = 0; i < this.state.objectiveList.length; i++) {
      let item = this.state.objectiveList[i]
      objectiveElems.push(
            ...
            <div className='column eight wide left aligned'>
              <form className='ui form'>
                ...
                <div className='field'>
                  ...
                  <div className='ui input'>
                    <textarea type='text' rows='10' maxLength='10000' name='funding' required='' value={this.state.fundingText} /> </div>
                </div>
              </form>
              <form className='ui form'>
